        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-8 text-gray-900">
                    <h2 class="text-2xl font-bold mb-6 text-slate-800 border-b pb-2">Editar Evento: {{ $event->title }}</h2>
                    ...
                    <form action="{{ route('events.update', $event) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="type" id="type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    <option value="workshop" {{ old('type', $event->type) == 'workshop' ? 'selected' : '' }}>Taller (Workshop)</option>
                                    <option value="course" {{ old('type', $event->type) == 'course' ? 'selected' : '' }}>Curso</option>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4"
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>{{ old('description', $event->description) }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea name="requirements" id="requirements" rows="3"
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('requirements', $event->requirements) }}</textarea>
                        ...
                        <div class="bg-slate-50 p-4 rounded-lg border border-slate-200">
                            ...
                            @if($event->image)
                                <div class="mb-4">
                                    ...
                                    <img src="{{ asset('storage/' . $event->image) }}" alt="Imagen actual" class="h-32 w-auto object-cover rounded-md shadow-sm border">
                                </div>
                            @endif
