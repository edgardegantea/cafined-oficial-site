<x-layouts.app xmlns:flux="http://www.w3.org/1999/html">
    ...
    <flux:breadcrumbs>
        ...
        <flux:breadcrumbs.item :href="route('admin.posts.index')">
            Publicaciones </flux:breadcrumbs.item>
        <flux:breadcrumbs.item>
            Nuevo
        </flux:breadcrumbs.item>
    </flux:breadcrumbs>
    ...
    <div class="card mt-5">
        ...
        <form action="{{ route('admin.posts.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-5 relative">
                <img id="imgPreview" class="w-full aspect-video object-cover object-center" src="https://static.vecteezy.com/system/resources/previews/004/141/669/non_2x/no-photo-or-blank-image-icon-loading-images-or-missing-image-mark-image-not-available-or-image-coming-soon-sign-simple-nature-silhouette-in-frame-isolated-illustration-vector.jpg">
            ...
            <flux:input class="mb-5" type="file" name="image_2" label="Imagen secundaria (Opcional)" accept="image/*" />

            <flux:select class="mb-5" label="Categoría" name="category_id"> @foreach($categories as $category)
            ...
            <flux:textarea class="mb-5" label="Resumen" name="excerpt" rows="2">{{ old('excerpt') }}</flux:textarea>

            <div>
                <p class="font-medium text-sm mb-1">Contenido de la publicación</p>
                <div id="editor" class="w-full">{!! old('content') !!}</div>
                <textarea class="w-full hidden" name="content" id="content" rows="10">{{ old('content') }}</textarea>
            </div>
            ...
            <div class="flex space-x-3 mt-5">
                ...
                <label class="flex items-center">
                    <input type="radio" name="is_published" value="1" @checked(old('is_published', 1) == 1)>
                    <span class="ml-1">Publicado</span>
                </label>
                <label class="flex items-center">
                    <input type="radio" name="is_published" value="0" @checked(old('is_published', 1) == 0)>
    ...
    @push('js')
        ...
        <script>
            ...
            // Inicialización de Quill y enlace con el textarea oculto
